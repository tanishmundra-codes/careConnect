...
const BookingSystem = () => {
    ...
    // This state will now be filled with data from your database
    const [counselors, setCounselors] = useState([]);

    // ✅ STEP 1: Fetch real counselor data from the backend when the component loads
    useEffect(() => {
        const fetchCounselors = async () => {
            try {
                // Use the full URL to your backend endpoint
                const response = await axios.get('http://localhost:5000/api/counselors');
                setCounselors(response.data.data); // Store the fetched counselors
            } catch (error) {
    ...
    const handleBooking = async (e) => {
        ...
        try {
            // ✅ STEP 2: Use the full URL and ensure IDs are correct
            const response = await axios.post("http://localhost:5000/api/meetings", {
                // Replace with a real student ID from your users collection for testing
                studentId: "68cd52bef6bc6bb1b6ca0221", 
                counselorId: selectedCounselor, // This is now a valid MongoDB _id
                topic: reason,
    ...
    if (showConfirmation) {
        // Find counselor data from the state for the confirmation screen
        const selectedCounselorData = counselors.find(c => c._id === selectedCounselor);
        
        return (
          // ... (Your existing confirmation UI, no changes needed here)
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <form onSubmit={handleBooking} className="space-y-8">
                    {/* ... (Urgency Level and Session Type sections remain the same) ... */}
                    {/* Urgency Level */}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {[
                                ...
                                <button
                                    ...
                                    className={`p-4 rounded-lg border-2 text-left transition-all ${urgency === option.value ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}
                                >
                    ...
                    <div className="bg-white rounded-xl shadow-sm p-6">
                        ...
                        <div className="grid gap-4">
                            {counselors.map((counselor) => (
                                <button
                                    key={counselor._id} // ✅ Use the real _id from the database
                                    type="button"
                                    onClick={() => setSelectedCounselor(counselor._id)} // ✅ Set the real _id in state
                                    className={`p-4 rounded-lg border-2 text-left transition-all ${selectedCounselor === counselor._id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}
                                >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {[
                                ...
                                <button
                                    ...
                                    className={`p-4 rounded-lg border-2 text-center transition-all ${sessionType === type.value ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}
                                >
                    ...
                    {/* ... (Date, Time, and Reason sections remain the same) ... */}
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {timeSlots.map((time) => (
                                    <button
                                        ...
                                        className={`p-2 rounded-lg border text-sm transition-all ${selectedTime === time ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 hover:border-gray-300'}`}
                                    >
                                        {time}
                                    </button>
                                ))}
